...
- **Inference Engine**: Interfaces with local **Ollama** (Gemma 3) via native **Fetch API**. It uses VLM reasoning to extract hierarchical medical entities (Dosage, RxNorm) from images without intermediate OCR layers.
...
- **Base UI Composition**: Use the `render` prop on triggers (e.g., `TooltipTrigger`, `DropdownMenuTrigger`) to avoid nested `<button>` hydration errors.
  - _Correct_: `<TooltipTrigger render={<Button ... />}>...</TooltipTrigger>`
